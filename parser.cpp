...
int isProductLine(string line){
	...
	std::regex product_line_regex1(R"rgx((\w{3} .+ \d+))rgx"),
			   product_line_regex2(R"rgx((.+ \d+ \w{3}))rgx");
	...
	if(std::regex_match(line,match,product_line_regex1)){
		std::cout<<"Product line, with tax code on left.\n";
		return 1;
	}

	if(std::regex_match(line,match,product_line_regex2)){
		std::cout<<"Product line, with tax code on right.\n";
		return 2;
	}
	...
	if(first_word.length()==3){
		std::cout<<"Tax Code on left. \n";
		return 1;
	}
	else{
		size_t last_space_pos = line.find_last_of(' ');

		stringstream lss (line.substr(last_space_pos));
		string last_word;
		lss>>last_word;

		if(last_word.length()==3){
			std::cout<<"Tax Code on right. "<<last_word<<"\n";
			return 2;
		}
	}*/
...
*/ 
void extractProductInfo(int tc_pos, string line){
	...
	if(tc_pos == 1){
		...
		cout<<"Product: "<<line.substr(0,price_pos)<<' ';
...
int main(){
	...
	for(size_t i=line_nr;i<input.size();++i){
		...
		if(input[i].length()>3){
			...
			if(tc_pos){
				extractProductInfo(tc_pos, input[i]);
